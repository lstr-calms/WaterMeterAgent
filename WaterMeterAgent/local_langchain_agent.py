...
# LangChain imports for tools and structure
...
# Local LangChain Tool Definitions
class CleanTextTool(BaseTool):
    name: str = "clean_text"
    description: str = "Remove noise and normalize text format for water meter reading parsing"
...
class ExtractNumbersTool(BaseTool):
    ...
    description: str = "Extract all numeric values from text, useful for finding water meter readings"
    ...
    def _run(self, text: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        return json.dumps(numbers)  # Return as JSON string for consistency

class MeterIdentificationTool(BaseTool):
    name: str = "identify_meter"
    description: str = "Identify water meter tag from text using fuzzy matching"
...
class StandardizeUnitTool(BaseTool):
    name: str = "standardize_unit"
    description: str = "Convert unit variations to standard form (cubic_meters, gallons, liters)"
...
class ValidateRangeTool(BaseTool):
    ...
    description: str = "Check if a water meter reading is within reasonable range for the given unit"
...
class ParseDateTool(BaseTool):
    name: str = "parse_date"
    description: str = "Extract date from text or return current date in YYYY-MM-DD format"
...
class LocalReActAgent:
    """
    Local ReAct agent that uses rule-based decision making with LangChain tools
    """
    
    def __init__(self):
        print("🔧 Initializing Local ReAct Agent...")
        ...
        print("✅ Local ReAct Agent initialized!")
    
    def parse_reading(self, raw_input: str) -> Dict[str, Any]:
        """
        Parse water meter reading using local ReAct pattern with LangChain tools
        """
        print(f"\n🚀 LOCAL REACT AGENT STARTING PARSE: '{raw_input}'")
        print("=" * 60)
    ...
    def _act(self, state: Dict[str, Any], action: str, step: int):
        """Execute the decided action using LangChain tools"""
        
        if action == "CLEAN_TEXT":
            result = self.tools['clean_text']._run(state['raw_input'])
            state['cleaned_text'] = result
            state['tool_calls'].append(('clean_text', state['raw_input'], result))

        elif action == "IDENTIFY_METER":
            result = self.tools['identify_meter']._run(state['cleaned_text'])
            ...
            result = self.tools['extract_numbers']._run(state['cleaned_text'])
            ...
            result = self.tools['standardize_unit']._run(state['cleaned_text'])
            state['unit'] = result
            state['confidence'] += 0.2
            state['tool_calls'].append(('standardize_unit', state['cleaned_text'], result))

        elif action == "PARSE_DATE":
            result = self.tools['parse_date']._run(state['cleaned_text'])
            state['date'] = result
            ...
            result = self.tools['validate_range']._run(validation_input)
    ...
    def _generate_result(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            'processing_method': 'local_react_langchain',
    ...
    def _display_results(self, result: Dict[str, Any]):
        ...
        print(f"\n🎯 LOCAL REACT AGENT FINAL RESULT:")
        print("=" * 60)
...
def main():
    ...
    """
    print("🤖 Local LangChain Water Meter Agent")
    print("(No APIs or LLMs - Pure Local Processing)")
    print("=" * 50)
    
    # Initialize local agent
    try:
        agent = LocalReActAgent()
    except Exception as e:
